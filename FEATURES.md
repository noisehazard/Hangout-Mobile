...
- **Soft-launch metrics** — the `/admin` screen leads with the five metrics from the supply plan (live hangouts per day, share of app opens with something in radius, hosts other than you in 14 days, repeat attendance, joins per hangout), each against its target and coloured green/red. A dash means no data yet, which is deliberately distinct from zero. The `metrics-digest` Edge Function sends the same numbers to the operator's Telegram weekly; it counts non-admin hosts, since a service-role call has no `auth.uid()`.
...
- **Diagnostics** — handled and unhandled client errors are logged to `client_errors` (account id, device model, OS version, error detail), readable only by admins and auto-deleted after 30 days. The `error-alert` Edge Function pushes crashes to Telegram immediately and a daily digest of everything else. Disclosed in the in-app privacy policy.
...
**Operator alerts:** `error-alert` and `metrics-digest` deliver to Telegram, not email (`supabase/functions/_shared/telegram.ts`). These only ever notify the operator, and email demanded a verified sender, a domain, and a provider willing to relay for an individual — none of which exist yet. Set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` as function secrets. Messages over Telegram's 4096-character limit are split on line boundaries. **User-facing mail — sign-in codes above all — is unaffected and still goes through Supabase Auth's own SMTP settings.**
